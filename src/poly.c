    ...
    numx_real_t       *result)
{
    ...
    if (!coeffs || !result)              return NUMX_ERR_NULL_PTR;
    if (degree > NUMX_MAX_POLY_DEGREE)   return NUMX_ERR_INVALID_ARG;
    ...
    numx_real_t        tol)
{
    ...
    if (!coeffs || !roots || !nroots)     return NUMX_ERR_NULL_PTR;
    if (degree == 0 || degree > NUMX_MAX_POLY_DEGREE) return NUMX_ERR_INVALID_ARG;
    if (tol <= (numx_real_t)0.0)          return NUMX_ERR_INVALID_ARG;

    for (i = 0; i <= degree; i++) work[i] = coeffs[i];
    ...
    while (deg >= 1) {
        ...
        for (s = 0; s < nstarts && !ok; s++) {
            ...
            for (iter = 0; iter < NUMX_MAX_ITER; iter++) {
                ...
                fx  = work[0]; dfx = (numx_real_t)0.0;
                for (i = 1; i <= deg; i++) {
                    dfx = dfx * x + fx;
                    fx  = fx  * x + work[i];
                }
                if (priv_abs(fx) < tol) { root = x; ok = 1; break; }
                if (priv_abs(dfx) < NUMX_EPSILON) break;
                step = fx / dfx;
                x -= step;
                if (priv_abs(step) < tol) { root = x; ok = 1; break; }
            }
        }

        if (!ok) break;  /* no real root found at this degree — likely complex pair */
