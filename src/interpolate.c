...
static numx_real_t priv_cos(numx_real_t x)
{
    ...
    x2 = x * x; term = (numx_real_t)1.0; sum = term;
    for (k = 1; k <= 12; k++) {
    ...
    numx_real_t       *result)
{
    ...
    if (!xs || !ys || !result)                  return NUMX_ERR_NULL_PTR;
    if (n < 2 || n > NUMX_MAX_INTERP_NODES)     return NUMX_ERR_INVALID_ARG;

    if (x <= xs[0])     { *result = ys[0];     return NUMX_OK; }
    if (x >= xs[n - 1]) { *result = ys[n - 1]; return NUMX_OK; }
    ...
    numx_real_t       *m)
{
    ...
    if (!xs || !ys || !m)                   return NUMX_ERR_NULL_PTR;
    if (n < 2 || n > NUMX_MAX_INTERP_NODES) return NUMX_ERR_INVALID_ARG;

    /* Natural boundary conditions: m[0] = m[n-1] = 0. */
    m[0] = m[n - 1] = (numx_real_t)0.0;

    if (n == 2) return NUMX_OK;  /* linear segment — no interior moments */
    ...
    for (i = n - 3; i >= 1; i--) {
        ...
        if (i == 0) break;  /* guard for unsigned wrap */
    }
    return NUMX_OK;
}
    ...
    numx_real_t       *result)
{
    ...
    if (!xs || !ys || !m || !result)         return NUMX_ERR_NULL_PTR;
    if (n < 2 || n > NUMX_MAX_INTERP_NODES)  return NUMX_ERR_INVALID_ARG;
    ...
    numx_real_t   *result)
{
    ...
    if (!f || !result)                       return NUMX_ERR_NULL_PTR;
    if (n < 2 || n > NUMX_MAX_INTERP_NODES)  return NUMX_ERR_INVALID_ARG;
    if (b <= a)                              return NUMX_ERR_INVALID_ARG;
    ...
    for (i = 0; i < n; i++) {
        dx = x - nodes[i];
        if (priv_abs(dx) < NUMX_EPSILON) { *result = fvals[i]; return NUMX_OK; }
        numx_real_t arg = NUMX_PI * (numx_real_t)(2*i + 1) / (numx_real_t)(2*n);
