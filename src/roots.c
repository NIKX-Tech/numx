    ...
    numx_real_t   *root)
{
    ...
    if (!f || !root)               return NUMX_ERR_NULL_PTR;
    if (tol <= (numx_real_t)0.0)  return NUMX_ERR_INVALID_ARG;

    fa = f(a); fb = f(b);
    if (fa * fb > (numx_real_t)0.0) return NUMX_ERR_INVALID_ARG;
    if (fa == (numx_real_t)0.0) { *root = a; return NUMX_OK; }
    if (fb == (numx_real_t)0.0) { *root = b; return NUMX_OK; }

    for (i = 0; i < NUMX_MAX_ITER; i++) {
        mid  = (a + b) * (numx_real_t)0.5;
        fmid = f(mid);
        if (priv_abs(fmid) < NUMX_EPSILON || (b - a) * (numx_real_t)0.5 < tol) {
            *root = mid; return NUMX_OK;
        }
        if (fa * fmid < (numx_real_t)0.0) { b = mid; fb = fmid; }
        else                               { a = mid; fa = fmid; }
    }
    ...
    numx_real_t   *root)
{
    ...
    if (!f || !df || !root)        return NUMX_ERR_NULL_PTR;
    if (tol <= (numx_real_t)0.0)  return NUMX_ERR_INVALID_ARG;

    x = x0;
    for (i = 0; i < NUMX_MAX_ITER; i++) {
        fx  = f(x);
        if (priv_abs(fx) < tol) { *root = x; return NUMX_OK; }
        dfx = df(x);
        if (priv_abs(dfx) < NUMX_EPSILON) return NUMX_ERR_SINGULAR;
        step = fx / dfx;
        x   -= step;
        if (priv_abs(step) < tol) { *root = x; return NUMX_OK; }
    }
    return NUMX_ERR_NO_CONVERGE;
}
    ...
    numx_real_t   *root)
{
    ...
    c = a; fc = fa; d = (numx_real_t)0.0; mflag = 1;

    for (i = 0; i < NUMX_MAX_ITER; i++) {
        if (priv_abs(fb) < NUMX_EPSILON || priv_abs(b - a) < tol) {
            *root = b; return NUMX_OK;
        }
        ...
        if ((s < qlo || s > qhi) || priv_abs(s - b) >= q * (numx_real_t)0.5
                                  || q < tol) {
        ...
        if (fa * fs < (numx_real_t)0.0) { b = s; fb = fs; }
        else                             { a = s; fa = fs; }
