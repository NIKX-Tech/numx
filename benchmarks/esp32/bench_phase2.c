...
static void ad_bench_cycle(void)
{
    ...
    numx_ad_var(&s_ad_tape, (numx_real_t)1.5, &xi);
...
static const numx_real_t fir_taps[8] = {
    (numx_real_t)0.0625, (numx_real_t)0.125, (numx_real_t)0.25, (numx_real_t)0.5, (numx_real_t)0.5, (numx_real_t)0.25, (numx_real_t)0.125, (numx_real_t)0.0625
};
static const numx_real_t iir_b[3] = { (numx_real_t)0.0675,  (numx_real_t)0.1349,  (numx_real_t)0.0675 };
static const numx_real_t iir_a[2] = {-(numx_real_t)1.1430,  (numx_real_t)0.4128 };
...
void bench_phase2(void)
{
    ...
    bench_section("autodiff/autodiff.md");
    {
        ...
        BENCH("dual_mul (x*x fwd-mode)",       BENCH_N_BIG, (
            xd = numx_dual_var((numx_real_t)1.5),
    ...
    for (int i = 0; i < 128; i++) fft64[i]  = (i % 2 == 0) ? (numx_real_t)(i/2 + 1) * (numx_real_t)0.1 : (numx_real_t)0.0;
    for (int i = 0; i < 512; i++) fft256[i] = (i % 2 == 0) ? (numx_real_t)(i/2 + 1) * (numx_real_t)0.1 : (numx_real_t)0.0;
    for (int i = 0; i < 1024; i++) fft512[i] = (i % 2 == 0) ? (numx_real_t)(i/2 + 1) * (numx_real_t)0.1 : (numx_real_t)0.0;
    ...
    /* Initialise buffers */
    for (int i = 0; i < 64; i++)  sig_x64[i]  = (numx_real_t)(i % 8) * (numx_real_t)0.5;
    for (int i = 0; i < 8; i++)   sig_h8[i]   = (numx_real_t)0.125;
    for (int i = 0; i < 128; i++) sig_x128[i] = (numx_real_t)(i % 16) * (numx_real_t)0.25;
    ...
    BENCH("signal_ema n=128 alpha=0.1",   BENCH_N_MED,
          numx_signal_ema(sig_x128, 128, (numx_real_t)0.1, sig_out128));

    /* ══════════════════════════════════════════════════════════════════ */
    printf("\n[sketch]\n");

    /* Initialise A matrices: diagonal-dominant values */
    for (int i = 0; i < 64; i++)  sk_A8[i]  = (i % 9 == 0) ? (numx_real_t)2.0 : (numx_real_t)0.1;
    for (int i = 0; i < 256; i++) sk_A16[i] = (i % 17 == 0) ? (numx_real_t)3.0 : (numx_real_t)0.1;
    ...
    for (int i = 0; i < 512; i++) cs_A[i] = (i % 17 == 0) ? (numx_real_t)1.0 : (numx_real_t)0.1;
    memset(cs_y, 0, sizeof(cs_y));
    cs_y[0] = (numx_real_t)1.0;
    cs_y[3] = (numx_real_t)0.5;
    ...
    BENCH("cs_ista 16x32 lam=0.1 iter=100", BENCH_N_TINY,
          numx_cs_ista(cs_A, cs_y, 16, 32, (numx_real_t)0.1, (numx_real_t)0.05, 100, cs_x));
}
